...
normalized_h1 = tflearn.batch_normalization(hidden1)

hidden2 = tflearn.fully_connected(normalized_h1, n_hidden2, activation='relu',name='hidden2')
normalized_h2 = tflearn.batch_normalization(hidden2)

hidden3 = tflearn.fully_connected(normalized_h2, n_hidden3,activation='relu',name = 'hidden3')
normalized_h3 = tflearn.batch_normalization(hidden3)

hidden4 = tflearn.fully_connected(normalized_h3, n_hidden4,activation='relu',name = 'hidden4')
normalized_h4 = tflearn.batch_normalization(hidden4)

softmax = tflearn.fully_connected(normalized_h4, n_outputs, activation='softmax', name ='output')
net = tflearn.regression(softmax)

# 모델 객체 생성
model = tflearn.DNN(net,checkpoint_path="train_model/hidden(18,20,24,9)_BatchNormalize.ckpt")

# 모델 학습
model.fit(x_train, y_train, validation_set=None, n_epoch=n_epochs,batch_size=batch_size)


model.save("train_model/hidden(18,20,24,9)_BatchNormalize.tfl")