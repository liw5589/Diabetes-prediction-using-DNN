...
hidden3 = tflearn.fully_connected(hidden2, n_hidden3,activation='relu', weights_init= he,name = 'hidden3')
hidden4 = tflearn.fully_connected(hidden3, n_hidden4,activation='relu', weights_init= he,name = 'hidden4')
softmax = tflearn.fully_connected(hidden4, n_outputs, activation='softmax', weights_init= he, name ='output')
net = tflearn.regression(softmax)

# 모델 객체 생성
model = tflearn.DNN(net,checkpoint_path="train_model/hidden(18,20,24,9)_weight_init.ckpt")

# 모델 학습
model.fit(x_train, y_train, validation_set=None, n_epoch=n_epochs,batch_size=batch_size)


model.save("train_model/hidden(18,20,24,9)_weight_init.tfl")